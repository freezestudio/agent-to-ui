...
import { z } from "zod";
import {
  ComponentCommonSchema, ChildListSchema,
...
export const ImageSchema = ComponentCommonSchema.extend({
  ...
  fit: z.enum(["contain", "cover", "fill", "none", "scaleDown"])
    .optional().default("fill"),
  variant: z.enum(["icon", "avatar", "smallFeature", "mediumFeature", "largeFeature", "header"])
    .optional().default("mediumFeature"),
}).merge(WeightProp).strict();
...
export const RowSchema = ComponentCommonSchema.extend({
  ...
  justify: z.enum(["start", "center", "end", "spaceBetween", "spaceAround", "spaceEvenly", "stretch"])
    .optional().default("start"),
  align: z.enum(["start", "center", "end", "stretch"])
    .optional().default("stretch"),
}).merge(WeightProp).strict();
...
export const TextFieldSchema = ComponentCommonSchema.merge(CheckableProps).extend({
  ...
  variant: z.enum(["shortText", "longText", "number", "obscured"])
    .optional().default("shortText"),
}).merge(WeightProp).strict();

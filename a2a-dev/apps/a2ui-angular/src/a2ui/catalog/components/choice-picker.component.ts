...
@Component({
  ...
  standalone: true,
  template: `<div style="display:flex;flex-direction:column;gap:6px">@if(labels){<label style="font-size:.85rem;color:#555">{{labels}}</label>}<div [class.chips]="isChips" style="display:flex;flex-direction:column;gap:4px">@for(opt of optionList;track opt.value){@if(isChips){<button class="a2ui-chip" [class.selected]="selectedValues().has(opt.value)" (click)="toggle(opt.value)">{{opt.label}}</button>}@else{<label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:.9rem"><input type="checkbox" [checked]="selectedValues().has(opt.value)" (change)="toggle(opt.value)"/><span>{{opt.label}}</span></label>}}</div></div>`,
  styles: [`.a2ui-chip{padding:6px 14px;border:1px solid #d0d0d0;border-radius:20px;background:#fff;cursor:pointer;font-size:.85rem}.a2ui-chip.selected{background:#1a1a2e;color:#fff;border-color:#1a1a2e}.chips{flex-direction:row!important;flex-wrap:wrap}`],

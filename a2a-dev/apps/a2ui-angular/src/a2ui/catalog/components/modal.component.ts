...
@Component({
  selector: "a2ui-modal",
  standalone: true,
  template: `<ng-container #triggerContainer></ng-container>
    @if (visible()) {
...
export class ModalComponent extends BaseComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    if (triggerId && this.triggerContainer)
      this.childRenderer.renderChild(this.triggerContainer, this.surfaceId, triggerId);
    if (contentId && this.contentContainer)
      this.childRenderer.renderChild(this.contentContainer, this.surfaceId, contentId);
  }
  close(): void {

...
export class DateTimeInputComponent extends BaseComponent {
  ...
  constructor() {
    ...
    if (raw) {
      try {
        ...
        if (!isNaN(d.getTime())) {
          this.dateValue.set(d.toISOString().slice(0, 10));
          this.timeValue.set(d.toISOString().slice(11, 16));
        }
      } catch {}
    }
  }
  ...
  private writeBack(): void {
    ...
    const combined =
      this.enableDate && this.enableTime
  ...
  onDateChange(e: Event): void {
    this.dateValue.set((e.target as HTMLInputElement).value);
    this.writeBack();
  }
  onTimeChange(e: Event): void {
    this.timeValue.set((e.target as HTMLInputElement).value);
    this.writeBack();
  }
}

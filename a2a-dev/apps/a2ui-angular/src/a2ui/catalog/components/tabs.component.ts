...
export class TabsComponent extends BaseComponent implements AfterViewInit {
  ...
  tabs: Array<{ title: string; child: string }> = Array.isArray(this.props["tabs"])
    ? (this.props["tabs"] as any[]).map((t: any) => ({
        title: this.binding.resolveString(t.title, this.surfaceId),
  ...
  private renderTab(): void {
    ...
    if (tab?.child && this.childContainer) {
      ...
      this.childRenderer.renderChild(this.childContainer, this.surfaceId, tab.child);
    }
  }
}

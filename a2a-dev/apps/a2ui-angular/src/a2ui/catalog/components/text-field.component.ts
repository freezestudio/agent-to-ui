...
@Component({
  ...
  template: `<div style="display:flex;flex-direction:column;gap:4px">
    ...
    @for (err of errors(); track err) {
      <small style="color:#d32f2f;font-size:.8rem">{{ err }}</small>
    }
  </div>`,
  styles: [
    `
...
export class TextFieldComponent extends BaseComponent {
  ...
  inputType =
    (
      { shortText: "text", longText: "text", number: "number", obscured: "password" } as Record<
        string,
        string
      >
    )[(this.props["variant"] as string) ?? "shortText"] ?? "text";
